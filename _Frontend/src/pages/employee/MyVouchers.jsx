...
export default function MyVouchers() {
  ...
  const count = (s) => vouchers.filter(v => v.status === s).length;
        ...
        <div className="col-sm-12">
          <div className="card">
            ...
            <div className="card-block">
              ...
              {/* Filter tabs */}
              <ul className="nav nav-tabs m-b-20" id="voucherTab">
                {TABS.map(s => (
                  <li className="nav-item" key={s}>
                    <a
                      className={`nav-link${filter === s ? ' active' : ''}`}
                      href="#!"
                      onClick={(e) => { e.preventDefault(); setFilter(s); }}
                    >
                      {s === 'ALL' ? 'All' : s.charAt(0) + s.slice(1).toLowerCase()}
                      {s !== 'ALL' && (
                        <span className="badge badge-secondary ml-1">{count(s)}</span>
                      )}
                    </a>
                  </li>
                ))}
                          ...
                          <td>
                            ...
                            {v.status === 'DRAFT' && (
                              <button
                                ...
                                onClick={() => navigate(`/employee/vouchers/${v.id}/edit`)}
                              >
